...
import { saveBusinessTrips, deleteBusinessTrip, saveTripRecords, deleteTripRecord, getNameResolutions, generateId } from '@/lib/storage';
...
import { TripHomonymResolverDialog } from './trip-homonym-resolver-dialog';
...
export function TripBoard({ onDataChange }: TripBoardProps) {
    ...
    // Dialog state for Homonym Resolution
    const [isHomonymResolverOpen, setIsHomonymResolverOpen] = useState(false);
    const [ambiguousTripsToResolve, setAmbiguousTripsToResolve] = useState<{ trip: BusinessTrip, candidates: TeamMember[] }[]>([]);
    const [pendingParsedData, setPendingParsedData] = useState<{ trips: BusinessTrip[], unknownNames: string[] } | null>(null);
    ...
    const getTodayColorClass = (color: TodayColor, isHeader: boolean) => {
        if (!isHeader) return 'bg-transparent'; // 본문(Grid) 영역은 배경색을 투명하게 처리, 출장 바가 가려지지 않게 함

        switch (color) {
            case 'yellow': return 'bg-yellow-100 dark:bg-yellow-900/30 font-bold';
            case 'green': return 'bg-green-100 dark:bg-green-900/30 font-bold';
            case 'blue': return 'bg-blue-100 dark:bg-blue-900/30 font-bold';
            case 'purple': return 'bg-purple-100 dark:bg-purple-900/30 font-bold';
            case 'red': return 'bg-red-100 dark:bg-red-900/30 font-bold';
            default: return '';
        }
    };
    ...
    const finishImport = useCallback(async (tripsToSave: BusinessTrip[], unknowns: string[]) => {
        await saveBusinessTrips(tripsToSave);
        refreshData();
        setActiveTab('attendance');

        let msg = `✅ 총 ${tripsToSave.length}건의 출장 정보로 교체되었습니다.`;
        if (unknowns.length > 0) {
            msg += `\n⚠️ 매칭되지 않은 이름: ${unknowns.length}명 (${unknowns.slice(0, 3).join(', ')}...)`;
        }
        setImportToast(msg);
        setTimeout(() => setImportToast(null), 5000);
    }, [refreshData]);

    const handleHomonymResolve = async (resolutions: Record<string, string>) => {
        if (!pendingParsedData) return;

        // Apply resolutions
        const finalTrips = pendingParsedData.trips.map(t => {
            if (resolutions[t.id]) {
                return { ...t, knoxId: resolutions[t.id] };
            }
            return t;
        });

        setIsHomonymResolverOpen(false);
        await finishImport(finalTrips, pendingParsedData.unknownNames);
        setPendingParsedData(null);
        setAmbiguousTripsToResolve([]);
    };
    ...
    const handleClipboardImport = useCallback(async () => {
        try {
            ...
            const { trips: parsedTrips, unknownNames, ambiguousTrips } = parseTripText(text, members);
            ...
            // Auto-resolve homonyms against existing trips in DB
            const needsResolution: { trip: BusinessTrip, candidates: TeamMember[] }[] = [];
            for (const ambig of ambiguousTrips || []) {
                const existing = trips.find(t =>
                    t.name === ambig.trip.name &&
                    t.startDate === ambig.trip.startDate &&
                    t.endDate === ambig.trip.endDate &&
                    t.purpose === ambig.trip.purpose &&
                    t.knoxId
                );

                if (existing && existing.knoxId) {
                    // Auto resolved! Find it in parsedTrips and update it
                    const tripInParsed = parsedTrips.find(t => t.id === ambig.trip.id);
                    if (tripInParsed) tripInParsed.knoxId = existing.knoxId;
                } else {
                    needsResolution.push(ambig);
                }
            }

            if (needsResolution.length > 0) {
                // Require user input
                setAmbiguousTripsToResolve(needsResolution);
                setPendingParsedData({ trips: parsedTrips, unknownNames });
                setIsHomonymResolverOpen(true);
                return; // Pause the process
            }

            // If everything is resolved or no ambiguous trips, save immediately
            await finishImport(parsedTrips, unknownNames);
    ...
    }, [members, trips, finishImport, refreshData]);
    ...
    return (
        <div className="absolute inset-0 flex flex-col min-h-0 bg-white dark:bg-gray-900">
                        ...
                        {viewMode === 'list' ? (
                            ...
                            >
                                <div className="min-w-full w-fit pb-32">
                                        ...
                                        {dates.map(date => {
                                            ...
                                            return (
                                                <div
                                                    ...
                                                    className={`flex-shrink-0 text-center text-xs py-1 transition-colors ${isWeekend ? 'bg-gray-100 dark:bg-gray-700/30' : ''} ${isToday ? getTodayColorClass(todayColor, true) + ' border-l border-r border-t border-gray-600 dark:border-gray-500 rounded-t-sm z-20 relative' : 'border-r border-gray-200 dark:border-gray-700 border-t border-transparent z-0'}`}
                                    ...
                                    {/* Member Rows */}
                                    {ganttMembers.map((member, rowIndex) => {
                                        const isLastRow = rowIndex === ganttMembers.length - 1;
                                                    ...
                                                    <div className="absolute inset-0 flex pointer-events-none">
                                                        {dates.map((date, i) => {
                                                            ...
                                                            return (
                                                                <div
                                                                    key={i}
                                                                    className={`flex-shrink-0 h-full transition-colors ${isWeekend ? 'bg-gray-50/50 dark:bg-gray-800/20' : ''} ${isToday ? getTodayColorClass(todayColor, false) + ` ${isLastRow ? 'border-l border-r border-b rounded-b-sm' : 'border-l border-r'} border-gray-600 dark:border-gray-500 z-20 relative` : 'border-r border-gray-200 dark:border-gray-800/50 z-0'}`}
                                                                    style={{ width: dayWidth }}
                                                                />
                                                            );
                                                        })}
            ...
            />
            <TripHomonymResolverDialog
                open={isHomonymResolverOpen}
                onOpenChange={setIsHomonymResolverOpen}
                ambiguousTrips={ambiguousTripsToResolve}
                onResolve={handleHomonymResolve}
            />
        </div>
    );
}
