...
export function CalendarView({
    ...
    const { teamMembers: contextMembers, businessTrips: contextTrips, tripRecords: contextTripRecords, updateTask, notes, deleteNote } = useData();
    ...
    const handleTaskDrop = async (e: React.DragEvent, targetTask: Task) => {
        ...
        if (note) {
            ...
            if (window.confirm(`'${note.title || '메모'}' 내용을 '${targetTask.title}' 일정에 병합하시겠습니까?\n\n(병합 후 원본 메모는 삭제됩니다)`)) {
                ...
                // Update Task
                const result = await updateTask(targetTask.id, {
                    notes: newNotes,
                    resourceUrl: newUrl,
                    tags: newTags
                });
